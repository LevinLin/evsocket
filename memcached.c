...
enum try_websock_handshake{
    handshake_succ = 0,
    handshake_invalid,  /*< data invalid in header */
    handshake_fail
};
enum try_websock_handshake websock_handshake(conn *c);
...
conn *conn_new(const int sfd, enum conn_states init_state,
	...
	//change DATA_BUFFER_SIZE(2048) to WEBSOCK_DATA_BUFFER_SIZE(4096)
        c->wsize = WEBSOCK_DATA_BUFFER_SIZE;
...
static void conn_shrink(conn *c) {
    ...
    //change DATA_BUFFER_SIZE(2048) to WEBSOCK_DATA_BUFFER_SIZE(4096)
    if (c->rsize > READ_BUFFER_HIGHWAT && c->rbytes < WEBSOCK_DATA_BUFFER_SIZE) {
        char *newbuf;
        ...
        //change DATA_BUFFER_SIZE(2048) to WEBSOCK_DATA_BUFFER_SIZE(4096)
        newbuf = (char *)realloc((void *)c->rbuf, WEBSOCK_DATA_BUFFER_SIZE);

        if (newbuf) {
            c->rbuf = newbuf;
            //change DATA_BUFFER_SIZE(2048) to WEBSOCK_DATA_BUFFER_SIZE(4096)
            c->rsize = WEBSOCK_DATA_BUFFER_SIZE;
        }
...
/*
 * rewrite try_read_command to work for websocket
 */
/*
static int try_read_command(conn *c) {
    assert(c != NULL);
...
enum try_websock_handshake websock_handshake(conn *c){
    ...
    const char *sec_websocket_key_low = "sec-websocket-key: ";
    const int websocket_magic_value_len = 24;
    ...
    p = p + strlen(sec_websocket_key);
    if (p+websocket_magic_value_len > c->rcurr+c->rbytes) {
        return handshake_invalid;
    }
    strncpy(auth, p, websocket_magic_value_len);
    q = auth + websocket_magic_value_len;
    strncpy(q, websocket_magic_key, strlen(websocket_magic_key));
    q = q + strlen(websocket_magic_key);
    *q = 0;
    unsigned int auth_len = websocket_magic_value_len+strlen(websocket_magic_key);
    unsigned int message_digest[5];     // 5*4(byte)=20byte
    SHA1Context sha1;
    SHA1Reset(&sha1);
    SHA1Input(&sha1, (const unsigned char *)auth, auth_len);
    if (0 == SHA1Result(&sha1)) {
        return handshake_fail;
    }
    int i;
    for (i = 0; i < 5; i++) {
        message_digest[i] = htonl(sha1.Message_Digest[i]);
    }
    char *acceptKey = NULL;
    if (base64_encode_alloc((const char *) message_digest, 20, &acceptKey) == 0) {
	return handshake_fail;
    }
    if (acceptKey == NULL) {
	return handshake_fail;
    ...
    int len2 = strlen("Sec-WebSocket-Accept: ");
    int len3 = strlen(acceptKey);
    int len4 = strlen("Sec-WebSocket-Protocol: chat\r\n");
    memcpy(c->wbuf, str1, len1);
    memcpy(c->wbuf+len1, "Sec-WebSocket-Accept: ", len2);
    memcpy(c->wbuf+len1+len2, acceptKey, len3);
    memcpy(c->wbuf+len1+len2+len3, "\r\n", 2);
    memcpy(c->wbuf+len1+len2+len3+2, "Sec-WebSocket-Protocol: chat\r\n", len4);
    memcpy(c->wbuf+len1+len2+len3+len4+2, "\r\n", 2);
    c->wbytes = len1 + len2 + len3 + len4 + 4;
    c->wcurr = c->wbuf;
    //free memory
    free(acceptKey);
    acceptKey = NULL;
    ...
    return handshake_succ;
}

/*
 * rewrite drive_machine to work for websocket
 */
static void drive_machine(conn *c) {
    bool stop = false;
    int sfd;
    ...
    while (!stop) {
		...
		//WEBSOCK_DATA_BUFFER_SIZE = 2048, need enlarge?
                //change DATA_BUFFER_SIZE(2048) to WEBSOCK_DATA_BUFFER_SIZE(4096)
                dispatch_conn_new(sfd, conn_new_cmd, EV_READ | EV_PERSIST,
                                     WEBSOCK_DATA_BUFFER_SIZE, tcp_transport);
            }
		...
		conn_set_state(c, conn_websock_handshake);
            }

            break;

        case conn_websock_handshake :
            {
		//printf("handshake+%d\n", count++);
	    	switch(websock_handshake(c)){
		    case handshake_succ :
                    	conn_set_state(c, conn_mwrite);
                        c->write_and_go = conn_websock_parse_head;
		        break;
